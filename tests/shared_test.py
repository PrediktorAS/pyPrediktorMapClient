...
def mocked_requests(*args, **kwargs):
    if args[0] == f"{URL}something":
        return MockResponse(return_json, 200, {"Content-Type": "application/json"})
...
class AnalyticsHelperTestCase(unittest.TestCase):
    ...
    def test_request_from_api_json_response(self, mock_get):
        ...
        mock_get.return_value = MockResponse(json_response, 200, headers={"Content-Type": "application/json"})
    ...
    def test_request_from_api_non_json_response(self, mock_get):
        ...
        mock_get.return_value = MockResponse(non_json_response, 200, headers={"Content-Type": "text/plain"})
        ...
        self.assertEqual(result, {"error": "Non-JSON response", "content": non_json_response})
        mock_get.assert_called_once_with(f"{URL}test", timeout=(3, 27), params=None, headers=None)
    ...
    def test_request_from_api_extended_timeout(self, mock_get):
        mock_get.return_value = MockResponse(return_json, 200, {"Content-Type": "application/json"})
        result = request_from_api(rest_url=URL, method="GET", endpoint="something", extended_timeout=True)
        mock_get.assert_called_with(f"{URL}something", timeout=(3, 300), params=None, headers=None)
        assert result == return_json

    @mock.patch("requests.Session")
    def test_request_from_api_with_session(self, mock_session):
        mock_session_instance = mock_session.return_value
        mock_session_instance.get.return_value = MockResponse(return_json, 200, {"Content-Type": "application/json"})

        session = requests.Session()
        result = request_from_api(rest_url=URL, method="GET", endpoint="something", session=session)

        mock_session_instance.get.assert_called_with(f"{URL}something", timeout=(3, 27), params=None, headers=None)
        assert result == return_json

    @mock.patch("requests.post")
    def test_request_from_api_with_params_and_headers(self, mock_post):
        mock_post.return_value = MockResponse(return_json, 200, {"Content-Type": "application/json"})
        params = {"param": "value"}
